        ...
        if invoice_form.is_valid():
            try:
                with transaction.atomic():
                    # Save invoice without total_amount initially
                    invoice = invoice_form.save(commit=False)

                    # Generate a new invoice number
                    latest_invoice = Invoice.objects.last()
                    if latest_invoice:
                        # Extract the number part from the invoice_number using regex
                        match = re.search(r'(\d+)$', latest_invoice.invoice_number)
                        if match:
                            number_part = int(match.group(1)) + 1  # Increment the last number
                            new_invoice_number = f"INV-{latest_invoice.invoice_number[4:10]}-{str(number_part).zfill(3)}"
                        else:
                            new_invoice_number = "INV-250125-001"  # Default if no match found
                    else:
                        new_invoice_number = "INV-250125-001"  # Default for the first invoice

                    # Set the new invoice number
                    invoice.invoice_number = new_invoice_number
                    invoice.save()

                    # Fetch products and quantities from the form
                    ...
                    # Save total amount to invoice
                    invoice.total_amount = total_amount
                    invoice.save()

                    # Success message
                    messages.success(request, f"Invoice {invoice.invoice_number} created successfully!")
                    return redirect('invoicelist')  # Redirect to the invoice list page
...
def invoice_detail(request, invoice_id):
    ...
    return render(request, 'invoice details.html', {'invoice': invoice, 'items': items})
...
def registration(request):
    if request.method == "POST":
        ...
        l.uid_id = id
        l.save()
        return HttpResponse("<script>alert('Registered successfully');window.location='/form';</script>")
